...
import { getHrIndexPath, loadHrKnowledgeIndex } from "./knowledge-loader";

const CANDIDATE_TOP_K = 18;
const PROMPT_TOP_K = 5;
const MIN_AVG_SIMILARITY = 0.22;

function preferredHrFilePatterns(query: string): RegExp[] {
  const q = query.toLowerCase();
  if (/onboarding|rekrut|yangi xodim|ishga qabul/.test(q)) return [/aq-hr_02/i];
  if (/kpi|performance|baholash|samara|xodimlarni boshqarish/.test(q)) return [/aq-hr_03/i];
  if (/motivatsiya|retention|madaniyat|ushlab/.test(q)) return [/aq-hr_04/i];
  if (/turnover|analitika|dashboard|hisobot|executive/.test(q)) return [/aq-hr_05/i];
  if (/strategiya|direktor|boshqaruv/.test(q)) return [/aq-hr_01/i];
  return [];
}

function rerankHrHits(query: string, hits: RetrievalHit[]): RetrievalHit[] {
  const preferred = preferredHrFilePatterns(query);
...
function logHrKnowledgeRetrieval(
  query: string,
...
export interface HrRetrievalResult extends AuditedRetrievalResult {
  knowledgeUsed: boolean;
  candidateCount: number;
}

export async function retrieveHrChunks(
  query: string,
  options: { topK?: number; forceRebuild?: boolean; log?: boolean } = {}
): Promise<HrRetrievalResult> {
  const index = await loadHrKnowledgeIndex(options.forceRebuild);
  const indexPath = getHrIndexPath();
  ...
  const reranked = dedupeHits(rerankHrHits(query, candidates.hits));
  ...
  if (options.log !== false) {
    logHrKnowledgeRetrieval(query, candidates, selected, knowledgeUsed);
  }
...
export function formatHrKnowledgeContext(result: HrRetrievalResult): string {
  if (!result.knowledgeUsed || !result.hits.length) {
    return "Mos HR hujjat bo'lagi topilmadi yoki similarity past.";
  }
