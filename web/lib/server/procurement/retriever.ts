...
import { getProcurementIndexPath, loadProcurementKnowledgeIndex } from "./knowledge-loader";

const CANDIDATE_TOP_K = 18;
const PROMPT_TOP_K = 5;
const MIN_AVG_SIMILARITY = 0.22;

function preferredProcurementFilePatterns(query: string): RegExp[] {
  const q = query.toLowerCase();
  if (/xarid|asos|commercial|taklif|tanlash|bp-02/.test(q)) return [/aq-02_1/i];
  if (/yetkazib|logistika|yetkazib berish|delivery/.test(q)) return [/aq-02_2/i];
  if (/ombor|zaxira|sla/.test(q)) return [/aq-02_3/i];
  if (/shartnoma|kpi|contract/.test(q)) return [/aq-02_4/i];
  if (/risk|tavsiya|hisobot|bahola/.test(q)) return [/aq-02_5/i];
  if (/ta'?minot|taminot|bp-05|aq-02/.test(q)) return [/aq-02/i];
  return [];
}

function rerankProcurementHits(query: string, hits: RetrievalHit[]): RetrievalHit[] {
  const preferred = preferredProcurementFilePatterns(query);
...
function logProcurementKnowledgeRetrieval(
  query: string,
...
export interface ProcurementRetrievalResult extends AuditedRetrievalResult {
  knowledgeUsed: boolean;
  candidateCount: number;
}

export async function retrieveProcurementChunks(
  query: string,
  options: { topK?: number; forceRebuild?: boolean; log?: boolean } = {}
): Promise<ProcurementRetrievalResult> {
  const index = await loadProcurementKnowledgeIndex(options.forceRebuild);
  const indexPath = getProcurementIndexPath();
  ...
  const reranked = dedupeHits(rerankProcurementHits(query, candidates.hits));
  ...
  if (options.log !== false) {
    logProcurementKnowledgeRetrieval(query, candidates, selected, knowledgeUsed);
  }
...
export function formatProcurementKnowledgeContext(result: ProcurementRetrievalResult): string {
  if (!result.knowledgeUsed || !result.hits.length) {
    return "Mos ta'minot hujjat bo'lagi topilmadi yoki similarity past.";
  }
